...
{if isset($news)}
    ...
    <img src="{$prestablog_theme_upimg|escape:'html':'UTF-8'}thumb_{$news->id|intval}.jpg?{$md5pic|escape:'htmlall':'UTF-8'}" class="news" alt="{$news->title|escape:'htmlall':'UTF-8'}"/>{/if}
	<a name="article"></a>
    <p class="date_blog-cat">{l s='Published :' mod='prestablog'}
	...
	<h1 class="blog-title">{$news->title|escape:'htmlall':'UTF-8'}</h1>
    ...
    <div id="blog_product_linked">
		...
		<div class="productslinks">
				<a href="{$article.link|escape:'htmlall':'UTF-8'}">
					{PrestaBlogContent return=$article.thumb}
                    <br/>
					{$article.name|escape:'htmlall':'UTF-8'}
				</a>
        </div>
		{/foreach}
    </div>
