...
class SubBlocksClass extends ObjectModel
{
	...
	public function registerTablesBdd()
	{
		...
		if (!Db::getInstance(_PS_USE_SQL_SLAVE_)->Execute('
		CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.$this->table.'_lang` (
		...
		if (!Db::getInstance(_PS_USE_SQL_SLAVE_)->Execute('
		CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.$this->table.'_categories` (
		`'.$this->identifier.'` int(10) unsigned NOT null,
		`categorie` int(10) unsigned NOT null,
		PRIMARY KEY (`'.$this->identifier.'`, `categorie`))
		ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=utf8'))
			return false;
	...
	public function deleteTablesBdd()
	{
		...
		if (!Db::getInstance(_PS_USE_SQL_SLAVE_)->Execute('DROP TABLE `'._DB_PREFIX_.$this->table.'_lang`'))
			return false;
		if (!Db::getInstance(_PS_USE_SQL_SLAVE_)->Execute('DROP TABLE `'._DB_PREFIX_.$this->table.'_categories`'))
			return false;
	...
	public static function getHookListe($id_lang = null, $only_actif = 0)
	{
		...
		FROM `'._DB_PREFIX_.self::$table_static.'` sb
		JOIN `'._DB_PREFIX_.self::$table_static.'_lang` sbl ON (sb.`'.self::$identifier_static.'` = sbl.`'.self::$identifier_static.'`)
	...
	public static function getTitleSubBlock($id, $id_lang)
	{
		...
		$value = Db::getInstance(_PS_USE_SQL_SLAVE_)->GetRow('
			SELECT sbl.`title`
			FROM `'._DB_PREFIX_.self::$table_static.'` sb
			JOIN `'._DB_PREFIX_.self::$table_static.'_lang` sbl
				ON (sb.`'.self::$identifier_static.'` = sbl.`'.self::$identifier_static.'`)
			WHERE
				sbl.`id_lang` = '.(int)$id_lang.'
			AND	sb.`'.self::$identifier_static.'` = '.(int)$id);
	...
	public static function getCategories($id_subblock, $only_actif = 0)
	{
		...
		FROM `'._DB_PREFIX_.self::$table_static.'_categories` as sbc
	...
	public static function delAllCategories($id)
	{
		Db::getInstance(_PS_USE_SQL_SLAVE_)->Execute(
			'DELETE FROM `'._DB_PREFIX_.'prestablog_subblock_categories` WHERE `id_prestablog_subblock`='.(int)$id);
	}

	public static function delAllCorrespondanceAfterDelCat($cat)
	{
		Db::getInstance(_PS_USE_SQL_SLAVE_)->Execute(
			'DELETE FROM `'._DB_PREFIX_.'prestablog_subblock_categories` WHERE `categorie`='.(int)$cat);
	}
	...
	public static function updateCategories($categories, $id)
	{
		if (count($categories) > 0)
			foreach ($categories as $value)
				Db::getInstance(_PS_USE_SQL_SLAVE_)->Execute('
					INSERT INTO `'._DB_PREFIX_.'prestablog_subblock_categories`
	...
	public static function updatePositions($new_positions, $hook_name)
	{
		foreach ($new_positions as $position => $id)
			Db::getInstance(_PS_USE_SQL_SLAVE_)->Execute('
				...
				WHERE `'.self::$identifier_static.'`='.(int)$id.'
